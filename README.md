# Free COMPPROG-12 Final Project Ideas
- Any contributions are appreciated ;D

## [Downloads](https://github.com/Charliezkie-cloud/Charles-COMPPROG-12-Final-Project-Ideas/releases)
**Or (with git)**
1. Clone the project ``git clone https://github.com/Charliezkie-cloud/Charles-COMPPROG-12-Final-Project-Ideas.git``
2. Open with idea ``idea Charles-COMPPROG-12-Final-Project-Ideas``
3. Enjoy!

**Status Legend**
| Emoji | Description |
| --- | --- |
| ✅ | Done |
| ⚠️ | Ongoing |
| ❌ | Not done |

## Projects Status
| Difficulty | Name | Status |
| --- | --- | --- |
| Beginner | Number Guessing Game | ✅ |
| Beginner | Simple Calculator | ✅ |
| Beginner | Student Grade Calculator | ✅ |
| Intermediate | Quiz Application | ✅ |
| Intermediate | Student Record Manager | ✅ |
| Intermediate | Banking System | ✅ |
| Advanced | Inventory System | ⚠️ |
| Advanced | Simple Login System | ⚠️ |
| Advanced | Task Manager | ⚠️ |
